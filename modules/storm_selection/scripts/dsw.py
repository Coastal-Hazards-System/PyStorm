"""storm_selection — post-selection DSW + HC reconstruction.

Author / POC : Norberto C. Nadal-Caraballo, PhD  <norberto.c.nadal-caraballo@usace.army.mil>

Compute Discrete Storm Weights and reconstruct hazard curves for a previously
selected storm subset (the selected_storms.csv emitted by run_storm_selection.py).

Usage
-----
  1. Run scripts/run_storm_selection.py first to produce selected_storms.csv.
...
CONFIG = {
    ...
    # at data/processed/outputs/optimal/selected_storms.csv to post-process
    # the optimal-mode growth-loop final iteration instead.
    "selected_csv":   str(_MODULE_ROOT / "data/processed/outputs/fixed/selected_storms.csv"),
    "output_dir":     str(_MODULE_ROOT / "data/processed/outputs/fixed"),
...
def _load_inputs(cfg):
    ...
    if "original_index" not in sel.columns:
        raise ValueError(
            ...
            "Re-run scripts/run_storm_selection.py to regenerate it."
        )
...
def main():
    from storm_selection.weights.dsw import (
