...
The `storm_selection` module implements an end-to-end, reproducible pipeline
...
1. A C++ kernel (`backend/engines/cpp/_kmedoids_cpp`) exposed through a
   pybind11 binding. Honours arbitrary forced medoids.
...
(pure BUILD, no SWAP), used for the optional Sub-RTCS stage when an
...
(`scripts/run_storm_selection.py`, CyHAN §5.3). It dispatches to one of
...
```
┌─────────────────────────────────────────────────────────────────────┐
│  preprocess.py  →  data/processed/tc_data.h5                        │
...
All diagnostic stages (α/β sweep, Sub-RTCS, HC verification, QBM) are
...
├─────────────────────────────────────────────────────────────────────┤
│  [7]  Re-select RTCS at k_selected   (sampling/kmedoids)            │
├─────────────────────────────────────────────────────────────────────┤
...
- The α/β sweep (§3.2 step [3]) and Sub-RTCS (§3.2 step [9]) are
...
Outputs are written to `data/processed/outputs/<mode>/` so that the two
modes never overwrite each other.

| File                       | Mode  | Contents                                                          |
|----------------------------|-------|-------------------------------------------------------------------|
| `selected_storms.csv`      | both  | Original-index, storm_id, TC parameters of the RTCS               |
| `selected_storms_sub.csv`  | fixed | Sub-RTCS (optional)                                               |
...
- All workflow logic is Python and resides in
  `backend/python/storm_selection/`. The binding is imported as a conduit
...
```bash
# 1. Ingest raw inputs → tc_data.h5
python scripts/preprocess.py

# 2. Select the RTCS
python scripts/run_storm_selection.py                # fixed-k (default)
python scripts/run_storm_selection.py --mode optimal # growth loop

# 3. (Optional) Post-selection DSW reconstruction
python scripts/dsw.py

# 4. (Optional) Build the C++ k-medoids accelerator
cd backend/engines/cpp
python build.py
```

Outputs land in `data/processed/outputs/fixed/` or
`data/processed/outputs/optimal/`.

---

## 8. Module Layout (CyHAN v1.1 §16.2)
...
├── scripts/
│   ├── run_storm_selection.py         canonical launcher (§5.3)
...
## 9. Compliance, CyHAN v1.1

| Requirement                                       | Status                              |
|---------------------------------------------------|-------------------------------------|
| §4.1  C++ engine isolated, no HTTP/UI/orch        | ✓ `backend/engines/cpp/`            |
| §4.2  Python orchestration in package form        | ✓ `backend/python/storm_selection/` |
| §5.2  Self-contained, no sibling-module imports   | ✓                                   |
| §5.3  Launcher `run_<name>.py` at module root     | ✓ `scripts/run_storm_selection.py`  |
| §5.4  `snake_case` module identifier end-to-end   | ✓ `storm_selection`                 |
| §16.4 Binding is a conduit, not an authority      | ✓ wired by `sampling/kmedoids.py`   |

CLI is the operative deployment posture in v1.1 (§7.3 / §7.4).
