#!/usr/bin/env python
"""build.py - standalone build helper for the _jdm pybind11 extension.

Author : Norberto C. Nadal-Caraballo, PhD  <norberto.c.nadal-caraballo@usace.army.mil>

Builds the C++ kernel defined in ``JDMBootstrap.hpp`` + ``jdm_bindings.cpp`` and
installs the resulting extension into the expanded orchestration package
(``backend/python/joint_distribution_model/``) per CyHAN v2.2 §16.2 / §16.5.
...
PYTHON_PKG_DIR = (HERE / ".." / ".." / "python" / "joint_distribution_model").resolve()

MODULE_NAME = "_jdm"
SOURCE_FILE = HERE / "jdm_bindings.cpp"
...
def _build_direct() -> None:
    ...
    BUILD_DIR.mkdir(exist_ok=True)
    obj_file = BUILD_DIR / "jdm_bindings.o"
