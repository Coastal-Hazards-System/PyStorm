...
(a Reduced Storm Suite, RSS) from a large JPM storm set, so the
...
---------------
  fixed    - single run with k = (pre-selected count) + k_additional. Emits
             ...
             QBM bias surface, optional sub-RSS).
...
# ── Storm type ───────────────────────────────────────────────────────────
# "tc"  - tropical cyclones (the implemented suite selection).
# "etc" - extratropical cyclones (PLACEHOLDER; not yet implemented: the same
#         k-medoids / DSW / HC selection would run on an ETC synthetic-storm set).
# Overridable from the command line: --storm-type {tc,etc}.
STORM_TYPE = "tc"
...
#                         included in the RSS; set to None to skip.
...
# reduced_storm_suite.workflows.{rss_selection, growth_evaluation}.
...
CONFIG = {
    ...
    # If k_sub_rss > 0, a smaller subset is selected from the initial RSS.
    # Modes (see defaults.py for the full list):
    #   within          - PAM k-medoids inside the initial RSS
    #   within_maximin  - greedy farthest-point inside the initial RSS
    #   additional      - re-run main selection at k = forced + k_sub_rss
    "k_sub_rss":     0, # 50,
    "sub_rss_mode": "within_maximin",
    ...
    # Visualises which X parameters span well in the selected RSS.
