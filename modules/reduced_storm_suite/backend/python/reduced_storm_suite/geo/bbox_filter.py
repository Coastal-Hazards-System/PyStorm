"""Bounding-box node/storm filtering for regional RSS workflows.

Author : Norberto C. Nadal-Caraballo, PhD  <norberto.c.nadal-caraballo@usace.army.mil>
"""

from __future__ import annotations

import re
from pathlib import Path

import numpy as np

from reduced_storm_suite.io.readers import load_array


# ---------------------------------------------------------------------------
# Haversine
# ---------------------------------------------------------------------------

def haversine_km(lat1: float, lon1: float,
                 lat2: np.ndarray, lon2: np.ndarray) -> np.ndarray:
    """Great-circle distance (km) between a single point and array(s) of points."""
    R = 6_371.0
    rlat1, rlon1 = np.radians(lat1), np.radians(lon1)
    rlat2, rlon2 = np.radians(lat2), np.radians(lon2)
    dlat = rlat2 - rlat1
    dlon = rlon2 - rlon1
    a = np.sin(dlat / 2) ** 2 + np.cos(rlat1) * np.cos(rlat2) * np.sin(dlon / 2) ** 2
    return R * 2 * np.arcsin(np.sqrt(a))


# ---------------------------------------------------------------------------
# Node coordinates
# ---------------------------------------------------------------------------

def load_node_coordinates(
    mat_path:    str | Path,
    variable:    str = "nodeID",
    node_id_col: int = 0,
    lat_col:     int = 2,
    lon_col:     int = 3,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Load node IDs + lat/lon columns from the nodeID .mat file."""
    arr, _, _ = load_array(Path(mat_path), varname=variable)
    node_ids  = arr[:, node_id_col].astype(np.int64)
    lats      = arr[:, lat_col]
    lons      = arr[:, lon_col]
    return node_ids, lats, lons


# ---------------------------------------------------------------------------
# Bounding-box node filter
# ---------------------------------------------------------------------------

def filter_nodes_in_bbox(
    store_node_ids: list[str],
    all_node_ids: np.ndarray,
    all_lats: np.ndarray,
    all_lons: np.ndarray,
    bbox: dict,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Return Y-column indices of nodes that fall within the bounding box."""
    id_to_idx = {nid: i for i, nid in enumerate(all_node_ids)}

    store_ids_int = np.array([int(s) for s in store_node_ids], dtype=np.int64)

    col_indices = []
    kept_lats = []
    kept_lons = []

    lat_min, lat_max = bbox["lat_min"], bbox["lat_max"]
    lon_min, lon_max = bbox["lon_min"], bbox["lon_max"]

    for col_j, nid in enumerate(store_ids_int):
        pos = id_to_idx.get(nid)
        if pos is None:
            continue
        lat, lon = all_lats[pos], all_lons[pos]
        if lat_min <= lat <= lat_max and lon_min <= lon <= lon_max:
            col_indices.append(col_j)
            kept_lats.append(lat)
            kept_lons.append(lon)

    col_indices = np.array(col_indices, dtype=int)
    order = np.argsort(col_indices)
    return col_indices[order], np.array(kept_lats)[order], np.array(kept_lons)[order]


def compute_geographic_medoid(lats: np.ndarray, lons: np.ndarray) -> tuple[float, float]:
    """Return the (lat, lon) of the point that minimises total haversine distance."""
    best_dist = np.inf
    best_i = 0
    for i in range(len(lats)):
        d = haversine_km(lats[i], lons[i], lats, lons).sum()
        if d < best_dist:
            best_dist = d
            best_i = i
    return float(lats[best_i]), float(lons[best_i])


# ---------------------------------------------------------------------------
# TC track loader
# ---------------------------------------------------------------------------

def storm_ids_from_track_dir(
    track_dir: str | Path,
    file_pattern: str,
) -> list[int]:
    """List storm IDs present in *track_dir*, parsed from filenames, ascending.

    Builds a regex from *file_pattern* (its numeric field becomes a capture
    group) and scans the directory for matching TROP files, returning their
    integer IDs sorted ascending. Per the dataset convention, this ascending
    order is the Y-row order, so the result doubles as the per-row storm_ids
    map for subset suites (gom / sa) whose IDs are non-contiguous. For a
    contiguous full suite it simply yields 1..N.
    """
    track_dir = Path(track_dir)
    field = re.search(r"\{[^}]*\}", file_pattern)
    if field is None:
        return []
    prefix = re.escape(file_pattern[:field.start()])
    suffix = re.escape(file_pattern[field.end():])
    rx = re.compile(f"^{prefix}(\\d+){suffix}$")
    ids: list[int] = []
    for f in track_dir.iterdir():
        m = rx.match(f.name)
        if m:
            ids.append(int(m.group(1)))
    return sorted(ids)


def _storm_id_to_int(raw) -> int | None:
    """Coerce a stored storm_id to its integer master-suite ID.

    Accepts an int, a numeric string ("65", "0065"), or a string with the ID
    embedded ("SACCS_JPM0065_TROP" → 65). Returns None when no integer can be
    extracted (that storm then loads as an empty track)."""
    if raw is None:
        return None
    if isinstance(raw, (int, np.integer)):
        return int(raw)
    s = str(raw).strip()
    try:
        return int(s)
    except ValueError:
        digits = re.findall(r"\d+", s)
        return int(digits[-1]) if digits else None


def load_tc_tracks(
    track_dir: str | Path,
    n_storms: int,
    file_pattern: str = "LACPR2_JPM{:04d}_TROP.txt",
    storm_ids: list | None = None,
) -> list[np.ndarray]:
    """Read ITCS TROP track files. One file per storm, aligned to Y rows.

    The returned list has one entry per Y-row storm (length n_storms), so the
    0-based positions returned by ``filter_storms_near_point`` index straight
    back into X / Y.

    Storm-number resolution
    -----------------------
    ``storm_ids`` given
        Each Y row's track file is named by its master-suite storm ID
        (``file_pattern.format(master_id)``). Required for subset datasets
        whose file numbers are non-contiguous master IDs (e.g. the SACS gom /
        sa subsets). Rows whose storm_id can't be parsed to an int load empty.
    ``storm_ids`` is None
        Falls back to sequential 1-based numbering (file i ← row i-1), correct
        for contiguous suites (NACCS na, la, pr, tx) where ID == row position.
    """
    track_dir = Path(track_dir)
    tracks: list[np.ndarray] = []

    if storm_ids is not None:
        storm_numbers = [_storm_id_to_int(s) for s in storm_ids]
    else:
        storm_numbers = list(range(1, n_storms + 1))

    for storm_num in storm_numbers:
        if storm_num is None:
            tracks.append(np.empty((0, 2), dtype=float))
            continue
        fname = track_dir / file_pattern.format(storm_num)
        if not fname.exists():
            tracks.append(np.empty((0, 2), dtype=float))
            continue
        try:
            pts = []
            with open(fname, "r") as fh:
                fh.readline()  # skip header
                for line in fh:
                    parts = line.split(",")
                    if len(parts) < 4:
                        continue
                    lat = float(parts[2].strip())
                    lon = float(parts[3].strip())
                    pts.append((lat, lon))
            tracks.append(np.array(pts, dtype=float) if pts else np.empty((0, 2)))
        except Exception:
            tracks.append(np.empty((0, 2), dtype=float))

    return tracks


# ---------------------------------------------------------------------------
# Storm-distance filter
# ---------------------------------------------------------------------------

def filter_storms_near_point(
    tracks: list[np.ndarray],
    center_lat: float,
    center_lon: float,
    max_dist_km: float = 200.0,
) -> np.ndarray:
    """Return 0-based storm indices whose track has at least one point
    within *max_dist_km* of (center_lat, center_lon)."""
    keep = []
    for i, trk in enumerate(tracks):
        if trk.shape[0] == 0:
            continue
        dists = haversine_km(center_lat, center_lon, trk[:, 0], trk[:, 1])
        if dists.min() <= max_dist_km:
            keep.append(i)
    return np.array(keep, dtype=int)


# ---------------------------------------------------------------------------
# All-node coordinate lookup  (for map)
# ---------------------------------------------------------------------------

def get_store_node_coords(
    store_node_ids: list[str],
    all_node_ids: np.ndarray,
    all_lats: np.ndarray,
    all_lons: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    """Return (lats, lons) arrays aligned with store_node_ids order."""
    id_to_idx = {nid: i for i, nid in enumerate(all_node_ids)}
    lats = np.empty(len(store_node_ids))
    lons = np.empty(len(store_node_ids))
    for j, sid in enumerate(store_node_ids):
        pos = id_to_idx.get(int(sid))
        if pos is not None:
            lats[j] = all_lats[pos]
            lons[j] = all_lons[pos]
        else:
            lats[j] = lons[j] = np.nan
    return lats, lons


# ---------------------------------------------------------------------------
# High-level orchestrator
# ---------------------------------------------------------------------------

def apply_bbox_filter(
    bbox_cfg: dict,
    h5_path: str | Path,
    output_dir: str | Path,
) -> dict:
    """Run the full bounding-box filter pipeline."""
    from reduced_storm_suite.io.store import read_store

    print("\n[0] Bounding-box filter ...")
    bbox = bbox_cfg["bbox"]

    print(f"    Loading node coordinates from {bbox_cfg['node_coord_source']} ...")
    all_node_ids, all_lats, all_lons = load_node_coordinates(
        bbox_cfg["node_coord_source"],
        bbox_cfg.get("node_coord_variable", "nodeID"),
        bbox_cfg.get("node_id_col", 1),
        bbox_cfg.get("lat_col", 2),
        bbox_cfg.get("lon_col", 3),
    )
    print(f"    Full coordinate table : {len(all_node_ids)} nodes")

    data = read_store(Path(h5_path))
    store_node_ids = data.node_ids
    n_storms = data.X.shape[0]

    all_store_lats, all_store_lons = get_store_node_coords(
        store_node_ids, all_node_ids, all_lats, all_lons)

    print(f"    Bounding box : lat [{bbox['lat_min']:.4f}, {bbox['lat_max']:.4f}]"
          f"  lon [{bbox['lon_min']:.4f}, {bbox['lon_max']:.4f}]")
    node_col_indices, bbox_lats, bbox_lons = filter_nodes_in_bbox(
        store_node_ids, all_node_ids, all_lats, all_lons, bbox)
    print(f"    Nodes in bbox : {len(node_col_indices)} / {len(store_node_ids)}")

    if len(node_col_indices) == 0:
        raise ValueError("No nodes found within the bounding box. "
                         "Check bbox coordinates.")

    n_bbox = len(bbox_lats)
    max_sample = 2000
    if n_bbox <= max_sample:
        sample_idx = np.arange(n_bbox)
    else:
        rng = np.random.default_rng(42)
        sample_idx = rng.choice(n_bbox, size=max_sample, replace=False)
    best_dist = np.inf
    best_i = 0
    for i in sample_idx:
        d = haversine_km(bbox_lats[i], bbox_lons[i], bbox_lats, bbox_lons).sum()
        if d < best_dist:
            best_dist = d
            best_i = i
    medoid_lat = float(bbox_lats[best_i])
    medoid_lon = float(bbox_lons[best_i])
    print(f"    Nodes medoid   : ({medoid_lat:.4f}, {medoid_lon:.4f})")

    track_dir = bbox_cfg["track_dir"]
    pattern = bbox_cfg.get("track_file_pattern", "LACPR2_JPM{:04d}_TROP.txt")
    print(f"    Loading TC tracks from {track_dir} ...")

    # Resolve the per-Y-row storm IDs used to name each track file.
    #   1. store storm_ids (authoritative) when present;
    #   2. else derive from the track filenames: Y row i == the i-th storm by
    #      ascending ID (dataset convention). This is exact for the SACS
    #      subsets (gom/sa) and reduces to 1..N for contiguous suites - but
    #      only when the file count matches the Y-row count.
    #   3. else fall back to sequential 1..N (load_tc_tracks default).
    if data.storm_ids is not None:
        storm_ids = data.storm_ids
        print(f"    Using {len(storm_ids)} storm IDs from store.")
    else:
        derived = storm_ids_from_track_dir(track_dir, pattern)
        if len(derived) == n_storms:
            storm_ids = derived
            contiguous = derived == list(range(1, n_storms + 1))
            kind = "contiguous 1..N" if contiguous else "non-contiguous subset"
            print(f"    Derived {n_storms} storm IDs from track filenames "
                  f"(ascending-ID order, {kind}).")
        else:
            storm_ids = None
            print(f"    WARNING: {len(derived)} track files matched but Y has "
                  f"{n_storms} rows - cannot align by filename. Falling back to "
                  f"sequential 1..N (valid only for a complete contiguous suite).")

    tracks = load_tc_tracks(track_dir, n_storms, pattern, storm_ids=storm_ids)
    loaded = sum(1 for t in tracks if t.shape[0] > 0)
    print(f"    Tracks loaded : {loaded} / {n_storms}")

    max_dist = bbox_cfg.get("max_track_dist_km", 200.0)
    storm_indices = filter_storms_near_point(tracks, medoid_lat, medoid_lon, max_dist)
    print(f"    Storms within {max_dist:.0f} km of medoid : "
          f"{len(storm_indices)} / {n_storms}")

    if len(storm_indices) == 0:
        raise ValueError(
            f"No storms found within {max_dist} km of bbox medoid. "
            "Increase max_track_dist_km or check bbox coordinates.")

    return {
        "node_col_indices": node_col_indices,
        "storm_indices":    storm_indices,
        "medoid_lat":       medoid_lat,
        "medoid_lon":       medoid_lon,
        "bbox_node_lats":   bbox_lats,
        "bbox_node_lons":   bbox_lons,
        "all_node_lats":    all_store_lats,
        "all_node_lons":    all_store_lons,
        "tracks":           tracks,
        "n_storms_total":   n_storms,
    }
