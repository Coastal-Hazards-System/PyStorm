...
def _is_mat73(path: Path) -> bool:
    """Return True iff `path` is a MATLAB v7.3 (HDF5-backed) .mat file.

    Prefers ``h5py.is_hdf5`` (authoritative) when h5py is available; otherwise
    falls back to scanning a generous 1 KiB prefix for the HDF5 magic
    signature. The previous 136-byte heuristic occasionally missed v7.3 files
    on slow / network filesystems (short reads, non-standard prefix length).
    """
    if _HAS_H5PY:
        try:
