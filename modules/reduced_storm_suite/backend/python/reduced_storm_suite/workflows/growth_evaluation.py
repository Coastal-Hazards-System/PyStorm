"""RSS Selection (optimal k) - RMSE-driven growth sweep.
...
The selected RSS is then carried through the same diagnostic suite as
...
from reduced_storm_suite.workflows.rss_selection import (
    _load_pipeline_data,
...
def run_growth_evaluation(cfg: Optional[dict] = None):
    """RSS Selection (optimal k) - full RMSE sweep + tolerance-driven pick.
    ...
    print(f"\n{'='*60}")
    print(f"  RSS Selection (optimal k) - RMSE-Driven Growth Sweep")
    ...
    if HC_bench is None:
        raise ValueError(
            "HC_bench is required for RSS Selection (optimal k). "
    ...
    plot_pca_yspace(
        ...
        title=f"Y-space PCA - Selected RSS (k = {k_selected})")
