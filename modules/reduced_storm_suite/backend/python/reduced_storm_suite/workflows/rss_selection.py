"""RSS Selection (fixed k) - Reduced Storm Suite selection workflow.
...
Public API
----------
  run_rss_selection(cfg)  ->  (indices, metrics)
"""
...
from reduced_storm_suite.config.defaults import RSS_SELECTION_DEFAULTS
...
def _splom(X, x_cols, forced, new_indices, cfg, out_dir, filename,
           sub_indices=None, sub_label="Sub-RSS"):
...
def run_rss_selection(cfg: Optional[dict] = None):
    ...
    """
    base = copy.deepcopy(RSS_SELECTION_DEFAULTS)
    if cfg:
        base.update(cfg)
    cfg = base
    ...
    if forced is not None:
        print(f"  RSS Selection (fixed k)")
        print(f"  Pre-selected : {len(forced)}  +  additional : {n_additional}"
              f"  =  total k : {k}")
    else:
        print(f"  RSS Selection (fixed k = {k})")
    print(f"{'='*60}")
    ...
    k_sub = cfg.get("k_sub_rss")
    if k_sub is not None and k_sub > 0:
        sub_mode = cfg.get("sub_rss_mode", "within")
        step += 1

        if sub_mode in ("within", "within_maximin"):
            if k_sub >= len(indices):
                print(f"\n[{step}] Sub-RSS skipped: k_sub_rss ({k_sub}) "
                      f">= initial RSS size ({len(indices)}).")
                sub_indices = None
            else:
                Z_initial = Z[indices]
                if sub_mode == "within":
                    print(f"\n[{step}] Sub-RSS selection - picking {k_sub} from "
                          f"the initial {len(indices)} RSS (mode='within', PAM) ...")
                    sub_local = select_kmedoids(
                        Z_initial, k_sub, cfg["random_seed"])
                    sub_label = "Sub-RSS (within)"
                else:
                    print(f"\n[{step}] Sub-RSS selection - picking {k_sub} from "
                          f"the initial {len(indices)} RSS "
                          f"(mode='within_maximin', greedy farthest-point) ...")
                    sub_local = select_maximin(
                        Z_initial, k_sub, cfg["random_seed"])
                    sub_label = "Sub-RSS (within_maximin)"
        ...
        if sub_indices is not None:
            ...
            plot_pca_yspace(
                ...
                title=f"Y-space PCA — {sub_label} from {len(indices)} RSS",
