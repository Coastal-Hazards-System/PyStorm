...
def plot_growth_history(history, cfg, out_dir):
    ...
    fig.suptitle("PyStorm-RSS Growth Loop — All Metrics vs Subset Size k",
...
def plot_rmse_vs_k(history_df, tolerance, k_selected, rmse_selected,
    ...
    ax.set_xlabel("Number of selected storms, k (RSS size)", fontsize=11)
    ax.set_ylabel("Global mean hazard-curve RMSE  (m)", fontsize=11)
    ax.set_title(
        "RSS Selection (optimal k): Global HC RMSE vs Subset Size",
...
def plot_pca_yspace(
    ...
    ax.set_title(f"PyStorm-RSS {title or 'Y-space PCA — PC 1 vs PC 2'}",
                 fontsize=12, fontweight="bold")
    ax.grid(alpha=0.3)
...
def plot_tc_splom(
    ...
    fig.suptitle(f"PyStorm-RSS {title}",fontsize=14, fontweight="bold", y=1.01)
...
# ---------------------------------------------------------------------------
# HC comparison: benchmark vs RSS reconstruction
# ---------------------------------------------------------------------------
...
def _plot_hc_grid(
    ...
    fig.suptitle(f"PyStorm-RSS {title}",fontsize=13, fontweight="bold", y=1.02)
...
def plot_hc_comparison(
    ...
    _plot_hc_grid(Y_sub, DSW_global, HC_bench, tbl_aer, nodes, out_dir,
                  ...
                  "HC Comparison — Benchmark vs RSS (Global DSW)",
                  show_nodal=False)

    _plot_hc_grid(Y_sub, DSW_global, HC_bench, tbl_aer, nodes, out_dir,
                  dry_thr, "hc_comparison_nodal.png",
                  "HC Comparison — Benchmark vs RSS (Nodal + Global DSW)",
                  show_nodal=True)
...
def plot_hc_qbm(
    ...
    if qbm_mode == "aer":
        qbm_label = "RSS (QBM-AER corrected)"
        title_suffix = "QBM-AER"
    else:
        qbm_label = "RSS (QBM-response corrected)"
    ...
    fig, axes = plt.subplots(nrows, ncols,
                             figsize=(5 * ncols, 4 * nrows), squeeze=False)
    fig.suptitle(f"PyStorm-RSS HC Comparison -- Benchmark vs DSW vs {title_suffix}",
