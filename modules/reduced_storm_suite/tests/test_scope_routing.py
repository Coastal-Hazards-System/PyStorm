...
Validates: (1) bad scope rejected; (2) bad mode rejected; (3) regional scope forces bbox to None; (4) local scope invokes the bbox filter when given; (5) local scope without a bbox skips the filter; (6) storm_type tc/etc dispatch (bad rejected; etc placeholder raises).
"""
...
def test_run_rejects_bad_storm_type():
    with pytest.raises(ValueError, match="storm_type"):
        rss_main.run(config={"output_dir": "x", "storm_type": "bogus"},
                     mode="fixed", scope="regional")


def test_etc_storm_type_is_placeholder():
    with pytest.raises(NotImplementedError):
        rss_main.run(config={"output_dir": "x", "storm_type": "etc"},
                     mode="fixed", scope="regional")
