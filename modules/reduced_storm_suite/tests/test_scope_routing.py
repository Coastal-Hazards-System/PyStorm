...
def test_run_rejects_bad_scope():
    from main_reduced_storm_suite import run
    with pytest.raises(ValueError, match="scope"):
        run(config={"output_dir": "x"}, mode="fixed", scope="nope")


def test_run_rejects_bad_mode():
    from main_reduced_storm_suite import run
    with pytest.raises(ValueError, match="mode"):
        run(config={"output_dir": "x"}, mode="nope", scope="local")
...
def test_regional_scope_forces_bbox_to_none(monkeypatch, tmp_path):
    ...
    import main_reduced_storm_suite as mod
