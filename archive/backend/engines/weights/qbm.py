...
----------
  ...
  correct_node_qbm(resp, DSW_global, HC_bench_node, bias_22_node,
                    tbl_aer, ..., qbm_mode)
      -> (cum_aer_or_corrected_aer, surge_or_corrected_surge)
"""
...
def _bench_inverse_aer(HC_bench_node, tbl_aer, surge_query):
    ...
    Interpolates with surge as independent variable and log(AER) as dependent.
    Returns NaN where surge_query falls outside the benchmark surge range.
    """
    ...
    # Sort by surge ascending for interpolation
    ...
    # Convert back from log-AER to AER, preserving NaN
    with np.errstate(invalid="ignore"):
        return np.exp(result)


# ---------------------------------------------------------------------------
# Per-node storm-level bias: response mode (legacy)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Per-node storm-level bias: AER mode
# ---------------------------------------------------------------------------

def _node_storm_bias_aer(resp, DSW_global, HC_bench_node, tbl_aer, dry_thr):
    """
    Compute raw log-AER bias at per-storm AER positions for a single node.

    For each storm j the corrected AER is H_bench_inverse(surge_j).
    The raw bias in log-AER space is:
        delta_j = log(cum_aer_j) - log(aer_corrected_j)

    Returns the same tuple shape as _node_storm_bias_response so the
    smoothing pipeline can be shared.
    """
    valid = (~np.isnan(resp)) & (~np.isnan(DSW_global)) & (resp > dry_thr)
    if valid.sum() < 2:
        return None, None, None, None
...
def _bias_via_intermediate_grid(cum_aer_s, overlap, bias_raw,
    ...
    Works for both response bias and log-AER bias -- the values are
    just numbers on the same AER grid.
...
def compute_qbm_bias(
    ...
    qbm_mode : str
        "aer"      (default) -- bias is in log-AER space (horizontal
                    correction; surge values untouched).
        "response" -- bias is in response space (vertical correction;
                    legacy mode).

    Returns
    -------
    bias_tbl : [m x N_AER]  bias at the standard 22-AER grid per node.
               In "aer" mode these are log-AER deltas; in "response" mode
               these are surge deltas.
    """
    k, m = Y_sub.shape
    n_aer = len(tbl_aer)
...
def correct_node_qbm(
    ...
    qbm_mode      : "aer" or "response"

    Returns
    -------
    For qbm_mode="response":
        cum_aer       : [n_valid] original cumulative AER positions
        surge_corr    : [n_valid] corrected surge values (green circles)

    For qbm_mode="aer":
        aer_corr      : [n_valid] corrected AER positions (green circles)
        surge         : [n_valid] original surge values (untouched)
    ...
    if qbm_mode == "aer":
        # AER correction: shift AER positions, leave surge untouched
        ...
        # Response correction (legacy): shift surge, leave AER untouched
