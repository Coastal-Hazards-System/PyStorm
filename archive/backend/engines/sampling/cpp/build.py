#!/usr/bin/env python
"""
Standalone build script for _kmedoids_cpp extension module.
...
def _build_setuptools():
    ...
    for f in lib_dir.rglob("_kmedoids_cpp*"):
        dest = HERE / f.name
        shutil.copy2(f, dest)
...
def _build_cmake():
    ...
    for ext in (".pyd", ".so", ".dylib"):
        for path in BUILD_DIR.rglob(f"_kmedoids_cpp*{ext}"):
            dest = HERE / path.name
            shutil.copy2(path, dest)
            print(f"[build.py] Installed {dest}")
            return

    for path in (BUILD_DIR / "Release").glob("_kmedoids_cpp*"):
        dest = HERE / path.name
        shutil.copy2(path, dest)
        print(f"[build.py] Installed {dest}")
        return
...
def _build_direct():
    ...
    output = HERE / f"_kmedoids_cpp{ext_suffix}"
    BUILD_DIR.mkdir(exist_ok=True)
    ...
    # On Windows, use lld to avoid MinGW ld bugs; statically link winpthread.
    if sys.platform == "win32":
...
def clean():
    ...
    for f in HERE.glob("_kmedoids_cpp*"):
