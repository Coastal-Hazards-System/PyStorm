...
def select_maximin(
    ...
    """
    Greedy farthest-point (maximin) selection — pure BUILD step, no SWAP.

    At each step picks the row whose distance to the nearest already-selected
    row is largest.  Maximizes minimum spacing rather than minimizing total
    within-cluster distance.  Tends to land on the boundary / extremes of the
    point cloud first, which is the opposite trade-off from PAM.

    Parameters
    ----------
    Z              : [n, d]  feature matrix (rows are points)
    k              : number of points to select
    seed           : RNG seed used only when forced_indices is empty
                     (controls the initial point)
    forced_indices : optional row indices to seed the selection with;
                     remaining slots are filled greedily by farthest-point.
