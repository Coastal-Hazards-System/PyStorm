...
def load_node_coordinates(
    ...
    Parameters
    ----------
    node_id_col : column index for the node IDs used as lookup keys
                  (default 0 = main / BE_22 node IDs)

    Returns
    -------
    node_ids : int64 array [N]  — node IDs from *node_id_col*
...
def filter_nodes_in_bbox(
    ...
    Parameters
    ----------
    store_node_ids : node IDs from the HDF5 store (strings, main / BE_22 IDs)
    all_node_ids   : full coordinate table node IDs (int64, must use same ID scheme)
