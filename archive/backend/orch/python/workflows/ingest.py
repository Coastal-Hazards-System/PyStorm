...
class Preprocessor:
    ...
    def _load_node_filter(
        ...
        Config keys:
          ...
          Y_node_id_col          : which column holds the main node IDs that
                                   should be stored in HDF5 output (default 0)

        Returns
        -------
        filter_indices : sorted int64 array of 0-based column indices into Y,
                         or None if not configured.
        main_node_ids  : int64 array of main node IDs (column 0) in the same
                         sorted order as filter_indices, or None.
        """
        cfg = self.cfg
        ...
        # Column 1: ADCIRC node IDs → 0-based indices for subsetting Y columns
        filter_col = int(cfg.get("Y_node_filter_col", 1))
        adcirc_ids = arr[:, filter_col].astype(np.int64)   # 1-based ADCIRC IDs
        indices    = adcirc_ids - 1                         # convert to 0-based

        # Column 0: main node IDs (BE_22) → stored in HDF5 as the node identity
        id_col   = int(cfg.get("Y_node_id_col", 0))
        main_ids = arr[:, id_col].astype(np.int64)

        # Sort by ADCIRC index; keep main IDs in the same row order
    ...
    def run(self) -> Path:
        ...
        if filter_indices is not None:
            ...
            node_ids = [str(i) for i in main_node_ids.tolist()]  # store main (col 0) IDs
