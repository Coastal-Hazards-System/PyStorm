...
def plot_pca_yspace(
    ...
    TC parameter SPLOM:
      ...
      green  : sub-RTCS storms (filled, when sub_indices is provided)

    Called twice per run: once for the initial state (pre-selected only) and
    once after selection is complete.  Optionally called a third time to show
    a smaller sub-RTCS chosen from the initial RTCS.
    """
    ...
    # Sub-RTCS — green filled circles (drawn on top to stand out)
...
def plot_tc_splom(
    ...
    green  : sub-RTCS storms             (filled, when sub_indices is not None)
    """
...
def plot_hc_qbm(
    ...
    qbm_mode controls the correction axis:
      "aer"      — AER positions are remapped; surge values untouched.
      "response" — Surge values are corrected; AER positions untouched.

    aer_mode controls the intermediate grid for bias computation:
    ...
    for ax_idx, node in enumerate(nodes):
        ...
        if valid.sum() >= 2:
            ...
            if aer_out is not None:
                # Plot if correction actually moved points
                if qbm_mode == "aer":
