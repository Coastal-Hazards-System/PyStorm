...
RTCS_SELECTION_DEFAULTS: dict = {
    ...
    # When k_sub_rtcs is set, a smaller RTCS is selected after the main
    # selection completes.  Three modes are supported:
    #   "within"          = PAM k-medoids restricted to the initial RTCS.
    #                       Density-fit objective; tends to under-represent
    #                       boundary / extreme bins (e.g. lowest Dp).
    #   "within_maximin"  = Greedy farthest-point selection restricted to the
    #                       initial RTCS.  Coverage-fit objective; preferred
    #                       when the goal is to span the parameter space
    #                       (boundaries always covered).
    #   "additional"      = Run k-medoids on the full pool with len(forced)
    #                       + k_sub_rtcs total medoids; sub-RTCS = forced + new.
    # Set k_sub_rtcs to None to skip the sub-selection stage entirely.
    "k_sub_rtcs":     None,
    ...
    "qbm_ramp_frac": 0.03,
    # QBM correction mode:
    #   "aer"      (default) = AER-based correction — remaps cumulative AER
    #               positions via inverse benchmark; surge values untouched.
    #   "response" = Response-based correction (legacy) — shifts surge values
    #               vertically to match benchmark; alters model outputs.
    "qbm_mode": "aer",
