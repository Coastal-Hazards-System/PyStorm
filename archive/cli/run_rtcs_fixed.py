...
BBOX_CONFIG = {
    ...
    "node_id_col": 0,        # column in the .mat array with main node IDs (BE_22)
...
CONFIG = {
    ...
    # Sub-RTCS — optional second-stage smaller subset.  Three modes:
    #   "within"         : PAM k-medoids restricted to the initial RTCS
    #                      (density fit; may miss parameter-space boundaries
    #                      such as the lowest Dp bin).
    #   "within_maximin" : Greedy farthest-point selection restricted to the
    #                      initial RTCS (coverage fit; boundaries always
    #                      represented — recommended for SPLOM diagnostics).
    #   "additional"     : len(pre-selected) + k_sub_rtcs total medoids,
    #                      picked from the full pool (sub-RTCS = forced + new).
    # Set k_sub_rtcs to None to skip the sub-selection stage.
    "k_sub_rtcs":     50,
